...
import { View, Text, ScrollView, TouchableOpacity, StyleSheet, RefreshControl, Alert, TextInput, ActivityIndicator } from 'react-native';
...
export default function LiveAttendanceScreen() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // A 10s poll means a transient failure shouldn't pop an Alert every cycle —
      // surface it inline instead, and only block the screen if there's no data yet.
  ...
  const statusIcons: Record<string, React.ReactNode> = {
    ...
    late: <Clock size={16} color={theme.colors.warning} />,
    absent: <XCircle size={16} color={theme.colors.error} />,
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.statsBar}>
        <View style={[styles.statsItem, { backgroundColor: theme.colors.brandTertiary }]}>
          <Text style={[styles.statsValue, { color: theme.colors.brandPrimary }]}>{data?.present || 0}</Text>
...
const styles = StyleSheet.create({
  ...
  lateBar: { height: 6, backgroundColor: theme.colors.warning },
  ...
  markAbsentBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8, borderWidth: 1, borderColor: theme.colors.error },
  markAbsentText: { fontSize: 12, fontWeight: '600', color: theme.colors.error },
